...
def draw_overlay(img_bgr: np.ndarray, result: AnalysisResult) -> np.ndarray:
    ...
    for est in result.used_estimates:
        ...
        cv2.putText(ov, lbl, (x1+2,y1-5),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.42, (15,15,15), 1, cv2.LINE_AA)
...
def draw_seg_overlay(img_bgr: np.ndarray, result: AnalysisResult) -> np.ndarray:
    ...
    ctrs,_ = cv2.findContours((mask>0.5).astype(np.uint8),
...
def draw_bev(img_bgr: np.ndarray, result: AnalysisResult) -> np.ndarray:
    """Bird-Eye-View (Homography 변환 미리보기)"""
    h, w = img_bgr.shape[:2]
    img_rgb = cv2.cvtColor(img_bgr, cv2.COLOR_BGR2RGB).copy()
    gp = result.ground_plane
    if gp and gp.valid and gp.H is not None:
        try:
            bev = cv2.warpPerspective(img_rgb, gp.H, (w, h))
            # 우하단 소형 미리보기
            small = cv2.resize(bev, (w//3, h//3))
            img_rgb[h-h//3:, w-w//3:] = small
            # 경계선
            cv2.rectangle(img_rgb, (w-w//3-2,h-h//3-2),(w-1,h-1),(100,255,100),2)
            cv2.putText(img_rgb,"BEV",(w-w//3+4,h-h//3+14),
...
def _draw_gauge(ax, depth_cm: float, level_color: str):
    max_d = 150.0
    levels_col = [(0,10,"#64B5F6"),(10,30,"#4CAF50"),(30,50,"#FDD835"),
                  (50,80,"#FF9800"),(80,120,"#F44336"),(120,150,"#9C27B0")]
    ax.barh([0],[max_d],height=0.6,color="#374151",edgecolor=SPINE)
    for lo,hi,c in levels_col:
        w=min(hi,depth_cm)-lo
        if w>0: ax.barh([0],[w],left=lo,height=0.6,color=c,alpha=0.88)
    for mark,lbl in [(0,"0"),(30,"주의"),(50,"경고"),(80,"위험"),(120,"매우위험"),(150,"150cm")]:
        ax.axvline(mark,color="#6B7280",lw=0.8,alpha=0.6)
        ax.text(mark,-0.38,lbl,ha="center",va="top",color="#9CA3AF",fontsize=7)
    tx=min(depth_cm+3,max_d*0.85)
    ax.text(tx,0,f"  {depth_cm:.1f}cm",va="center",color="white",fontsize=12,fontweight="bold")
...
def _draw_summary(ax, result: AnalysisResult):
    lines = [
        f"{'─'*100}",
        f"  🌊 침수 여부: {'침수 감지됨 ⚠️ ' if result.is_flooded else '정상 ✅        '}"
    ...
    if result.outlier_estimates:
        lines.append(f"\n  ⚠️  IQR 제거 ({len(result.outlier_estimates)}개): "
                     + ", ".join(f"{e.source}:{e.depth_cm:.1f}cm" for e in result.outlier_estimates))
    if result.ground_plane and result.ground_plane.valid:
        gp = result.ground_plane
        lines.append(f"\n  🗺️  GroundPlane | scale:{gp.scale_cm_per_px:.3f}cm/px | "
    ...
    if result.warnings:
        lines.append(f"\n  ⚠️  경고: " + " | ".join(result.warnings))
    lines.append(f"\n  ⏱️  처리 시간: {result.proc_time:.2f}초")
...
def visualize(img_bgr: np.ndarray, result: AnalysisResult, out_path: str):
    img_rgb = cv2.cvtColor(img_bgr, cv2.COLOR_BGR2RGB)
    H, W    = img_rgb.shape[:2]
    ...
    ax1.set_title("① 원본",color=TC,fontsize=11,fontweight="bold",pad=8)
    ...
    if result.flood_mask is not None:
        seg_img=draw_seg_overlay(img_bgr,result)
        bev_img=draw_bev(seg_img[...,::-1],result)  # BGR→function expects bgr
        ax3.imshow(bev_img); title3="③ 침수 Seg + BEV 미리보기"
    else:
        ax3.imshow(img_rgb)
    ...
    if used:
        ...
        for bar,conf in zip(bars,confs):
            ax4.text(bar.get_x()+bar.get_width()/2,bar.get_height()+mx*0.02,
    ...
    _draw_gauge(ax5,result.weighted_depth_cm,result.level_color)
    ax5.set_title(f"⑤ 최종 수위 게이지 (불확실도 ±{result.uncertainty_cm:.1f}cm)",
    ...
    flood_tag="⚠️  침수 감지됨" if result.is_flooded else "✅  정상"
    fig.suptitle(
        f"침수 감지 & 수위 측정 시스템  v4.0  │  {flood_tag}  │  "
